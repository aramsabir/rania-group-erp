...
export class StaffVisitReportComponent implements OnInit {
  bercumberRoutes: any = [
    { icon: 'feather feather-home', route: '/home', name: 'Home' },
    { icon: 'feather feather-home', route: '/group', name: 'Group' },
    { icon: 'feather feather-home', route: '/group/reporting', name: 'Reporting & analysis' },
    // {
    //   icon: 'fe fe-codepen',
    //   route: '/visits',
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (this.p.year == undefined || !this.p.year) {
        this.router.navigate([], { queryParams: { year: new Date().getFullYear() } })
      }
  ...
  search(event: any) {
    this.router.navigate([], { queryParams: { year: event,unit:this.p.unit ,staff_id:this.p.staff_id} })

  }
  searchUnit(event: any) {
    this.router.navigate([], { queryParams: { year: this.p.year,unit:event ,staff_id:this.p.staff_id} })

  }
  searchStaff(event: any) {
    this.router.navigate([], { queryParams: { year: this.p.year,unit:this.p.unit,staff_id:this.p.staff_id } })
  ...
  getTotalVisitForCompanies() {
    this.DataIsLoading = true;
      this.http.call('group/reporting/staff_visit', ApiMethod.GET, this.p).subscribe((ptr: any) => {
      ...
      this.chartOptionVisitPercentage = {
          ...
          series: {
            ...
            dataLabels: {
              ...
              formatter: function (point) {
                console.log(point);
                return '<div style="font-size:14px"><b>' + point.name + '</b></div><div><b>:' + point.y + ' </b></div>';
              }
            }
          }
        },
        ...
        series: [
          //       {
          //     data: this.totalVisitForCompanies.hours
          //   },
          {
      ...
      if (ptr.status) {
          this.data_detail = ptr.data_detail
          this.total_data = ptr.total_data
          this.totalVisitForCompanies = ptr.data
          this.chartOptionVisitPercentage = {
            chart: {
              type: 'pie',
              renderTo: 'container',
            ...
            series: [
              ...
              //   },
              {
                name: this.dic.translate("#"),
                colorByPoint: true,
                data: this.totalVisitForCompanies
