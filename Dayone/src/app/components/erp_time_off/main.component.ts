...
export class MainComponent implements OnInit {
  ...
  getData() {
    this.httpService
    .call(`${'my-grouped-allocations'}`, ApiMethod.GET, this.params)
    .subscribe(
      (res: any) => {
        if (res.status) {
    ...
    this.httpService
      .call(`${'my-time-off-per-year'}`, ApiMethod.GET, { year: this.year })
      .subscribe(
        (res: any) => {
          if (res.status) {
  ...
  save() {
    this.httpService
      .call(`${'time-off'}`, ApiMethod.POST, this.params, this.modalData)
      .subscribe(
        (res: any) => {
          if (res.status) {
