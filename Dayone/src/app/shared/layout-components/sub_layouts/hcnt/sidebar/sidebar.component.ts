...
@Component({
  selector: 'app-hcnt-sidebar',
  templateUrl: './sidebar.component.html',
  styleUrls: ['./sidebar.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SidebarHCNTComponent {
  ...
  checkNavActiveOnLoad() {
    ...
    this.httpService.call('userinfo', ApiMethod.GET, {}).subscribe((res: any) => {
      if (res.status == true) {
        ...
        this.menuItems.push(
          {
            title: this.dic.translate('Home'), status: true, icon: 'fe fe-home', type: 'link', path: '/hcnt', badgeType: 'success', badgeValue: '2', active: false,
          },
        )
        if (resources.includes('hcnt:sales-per-month'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per month'), status: true, icon: 'fe fe-calendar', type: 'link', path: '/hcnt/sales-per-month',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('hcnt:sales-per-customer'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per customer'), status: true, icon: 'fe fe-user', type: 'link', path: '/hcnt/sales-per-customer',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('hcnt:sales-per-date'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per date'), status: true, icon: 'fa fa-calendar', type: 'link', path: '/hcnt/sales-per-date',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('hcnt:sales-per-product-type'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per product type'), status: true, icon: 'fa fa-dollar', type: 'link', path: '/hcnt/sales-per-product-type',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
