...
@Component({
  selector: 'app-abf-sidebar',
  templateUrl: './sidebar.component.html',
  styleUrls: ['./sidebar.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SidebarAbfComponent {
  ...
  checkNavActiveOnLoad() {
    ...
    this.httpService.call('userinfo', ApiMethod.GET, {}).subscribe((res: any) => {
      if (res.status == true) {
        ...
        this.menuItems.push(
          {
            title: this.dic.translate('Home'), status: true, icon: 'fe fe-home', type: 'link', path: '/aso-bricks', badgeType: 'success', badgeValue: '2', active: false,
          },
        )
        if (resources.includes('abf:sales-per-month'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per month'), status: true, icon: 'fe fe-calendar', type: 'link', path: '/aso-bricks/sales-per-month',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('abf:sales-per-customer'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per customer'), status: true, icon: 'fe fe-user', type: 'link', path: '/aso-bricks/sales-per-customer',queryParams:{from_date:this.from,to_date:this.to,customer_type:"dealer"}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('abf:sales-per-gov'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per gov'), status: true, icon: 'fa fa-map-marker', type: 'link', path: '/aso-bricks/sales-per-gov',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('abf:sales-per-brick-type'))
          this.menuItems.push(
            {
              title: this.dic.translate('Sales per brick type'), status: true, icon: 'fa fa-dollar', type: 'link', path: '/aso-bricks/sales-per-brick-type',queryParams:{from_date:this.from,to_date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('abf:inventory-now'))
          this.menuItems.push(
            {
              title: this.dic.translate('Inventory now'), status: true, icon: 'fa fa-th', type: 'link', path: '/aso-bricks/inventory-now', badgeType: 'success', badgeValue: '2', active: false,
            },
          )
        if (resources.includes('abf:inventory-and-sold-per-date'))
          this.menuItems.push(
            {
              title: this.dic.translate('Inventory and sold'), status: true, icon: 'fa fa-th', type: 'link', path: '/aso-bricks/inventory-and-sold',queryParams:{date:this.to}, badgeType: 'success', badgeValue: '2', active: false,
            },
          )
